...
SERVICE_NAME="swift-sft-v5-tool-dialogue-gpu5.service"
SERVICE_DIR="$HOME/.config/systemd/user"
SERVICE_FILE="$SERVICE_DIR/$SERVICE_NAME"
ENV_FILE="$PROJECT_ROOT/services/swift-sft-v5-tool-dialogue-gpu5.env"
LOG_DIR="$PROJECT_ROOT/tracking/logs"
SERVICE_LOG="$LOG_DIR/swift_sft_v5_tool_dialogue_gpu5.service.log"
...
[Unit]
Description=Swift GPU5 v5.1 Tool-Dialogue Migration SFT + Gate
...
echo "[INFO] run log pattern: $PROJECT_ROOT/tracking/logs/swift_sft_v5_1_tool_dialogue_gpu5_*.log"
echo "[INFO] start: systemctl --user start $SERVICE_NAME"
echo "[INFO] status: systemctl --user status $SERVICE_NAME --no-pager -n 80"
echo "[INFO] follow: journalctl --user -u $SERVICE_NAME -f"
