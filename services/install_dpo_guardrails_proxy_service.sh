...
SERVICE_NAME="dpo-guardrails-proxy.service"
SERVICE_DIR="$HOME/.config/systemd/user"
SERVICE_FILE="$SERVICE_DIR/$SERVICE_NAME"
RUNTIME_DIR="$PROJECT_ROOT/runtime/gpu7_dpo_guardrails_proxy"
ENV_FILE="$PROJECT_ROOT/services/dpo-guardrails-proxy.env"
...
[Unit]
Description=DPO Guardrails Proxy (OpenAI-compatible)
...
ExecStart=/usr/bin/python3 -m uvicorn scripts.dpo_guardrails_proxy:app --host 0.0.0.0 --port 8010 --workers 4 --app-dir $PROJECT_ROOT
Restart=always
RestartSec=2
...
echo "[INFO] endpoint: http://127.0.0.1:8010/v1"
