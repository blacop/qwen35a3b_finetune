...
DEFAULT_BASE = (
    ROOT
    / "datasets"
    / "sft_openai_messages.cleaned.v3_merged.single75_exact.repair_v3.tydata_pdfcurated.nometa.marketingfix.20260425.jsonl"
)
DEFAULT_REPAIR = ROOT / "templates" / "repair_no_ticket_no_repeat_json_sft_template.jsonl"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Merge cleaned SFT data with no-ticket/no-repeat/no-JSON hotfix samples.")
    ...
    parser.add_argument("--output-dir", type=Path, help="If set, auto-generate filenames under this directory.")
...
def main() -> None:
    ...
    if args.output_dir:
        ...
        output_path = output_dir / f"{base_path.stem}.nohallucination_hotfix_merged.jsonl"
        summary_path = output_dir / "nohallucination_hotfix_merge_summary.json"
    else:
        output_path = args.output or base_path.with_name(f"{base_path.stem}.nohallucination_hotfix_merged.jsonl")
