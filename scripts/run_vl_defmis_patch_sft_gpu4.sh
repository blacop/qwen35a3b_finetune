...
PROD_VL_SERVICE="${PROD_VL_SERVICE:-vllm-qwen35vl-gpu4.service}"
...
if systemctl --user is-active --quiet "${PROD_VL_SERVICE}"; then
  log "stopping ${PROD_VL_SERVICE} before VL def+mis patch SFT"
  systemctl --user stop "${PROD_VL_SERVICE}"
fi

wait_gpu_free "${CUDA_VISIBLE_DEVICES}"
log "gpu ${CUDA_VISIBLE_DEVICES} is free, starting VL definition+misconception patch SFT"
