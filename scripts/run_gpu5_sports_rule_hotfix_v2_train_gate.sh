...
PACK_DIR="${PACK_DIR:-$PROJECT_ROOT/datasets/sports_rule_hotfix_v2_${TS}}"
DATA_FILE="${DATA_FILE:-$PACK_DIR/sft_sports_rule_hotfix_v2.jsonl}"
...
FAILURE_CSV="${FAILURE_CSV:-$PROJECT_ROOT/eval_outputs/rule18_candidate_disable_thinking_20260429T062946Z/rule_check/rule_check_failures.csv}"

RUN_NAME_PREFIX="${RUN_NAME_PREFIX:-swift_sft_sports_rule_hotfix_v2_gpu5}"
...
log "sports rule hotfix v2 train+gate start"
...
python3 "$PROJECT_ROOT/scripts/build_sports_rule_hotfix_v2_pack.py" \
...
EVAL_MODEL_NAME="${EVAL_MODEL_NAME:-qwen35a3b-domain-text-vl-gpu5-rule-hotfix-v2-candidate}" \
EVAL_WORKERS="${EVAL_WORKERS:-3}" \
EVAL_MAX_TOKENS="${EVAL_MAX_TOKENS:-260}" \
...
SYSTEM_PROMPT="${SYSTEM_PROMPT:-你是体育包网客服。只输出最终客服回复，禁止输出推理过程、题目理解、首先/接下来、<think>。纯规则解释题直接说明规则和计算方法；只有具体注单、资金、赔率争议或账户异常才引导后台查询。不要营销、不要引流、不要承诺包赢。}" \
